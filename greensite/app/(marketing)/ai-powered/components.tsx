...
import { Experience } from '@/lib/types/ai-powered'
import { EXPERIENCES, RESUME_MATCH_BULLETS, SKILLS } from '@/lib/utils/ai-powered'
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between">
        <span className="text-[13px] font-semibold" style={{ color: '#1f2937' }}>{experience.role}</span>
        <span className="text-[10px]" style={{ color: '#9ca3af' }}>{experience.yr}</span>
      </div>
      <div className="text-[11px]" style={{ color: '#6b7280' }}>{experience.co}</div>
    </div>
  )
}
    ...
    <div className="flex-shrink-0 relative" style={{ width: 340 }}>
      <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ background: '#ffffff', border: '1px solid rgba(255,255,255,0.1)', padding: '24px' }}>
        ...
        <div className="mb-5">
          ...
          {EXPERIENCES.map((e) => (
            <ExperienceRow key={e.role} experience={e} />
          ))}
        </div>
        <div className="mb-5">
