    ...
    <div className="relative w-full h-[500px] flex items-center justify-center perspective-[1500px]">
      <div className="relative w-64 h-64 preserve-3d animate-master-rotate">
        {pieces.map((piece) => (
          <div
            key={piece.id}
            className={`absolute inset-0 preserve-3d transition-transform duration-700 assemble-piece-${piece.id}`}
            style={{
              // Hover effect: further separation along piece's natural path
              transform: hoveredPiece === piece.id
                ? `translate3d(${piece.dir.x * 30}px, ${piece.dir.y * 30}px, ${piece.dir.z * 30}px) scale(1.05)`
                : ''
            }}
      ...
      <style jsx>{`
        ...
        /* Exploded to Assembled animation sequence */
        ${pieces.map(p => `
          .assemble-piece-${p.id} {
            animation: assemble${p.id} 14s ease-in-out infinite;
          }
          @keyframes assemble${p.id} {
            0%, 15%, 85%, 100% { transform: translate3d(${p.dir.x * 160}px, ${p.dir.y * 160}px, ${p.dir.z * 160}px) rotateX(${p.dir.x * 15}deg) rotateY(${p.dir.y * 15}deg); opacity: 0.75; }
            45%, 65% { transform: translate3d(0, 0, 0) rotateX(0deg) rotateY(0deg); opacity: 1; }
          }
        `).join('\n')}
      `}</style>
    </div>
